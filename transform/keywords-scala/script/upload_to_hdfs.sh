...
docker exec $NAMENODE_CONTAINER hdfs dfs -mkdir -p $HDFS_BASE_PATH/giai_tri 2>/dev/null || true
docker exec $NAMENODE_CONTAINER hdfs dfs -mkdir -p $HDFS_BASE_PATH/cong_nghe 2>/dev/null || true
docker exec $NAMENODE_CONTAINER hdfs dfs -mkdir -p $HDFS_BASE_PATH/suc_khoe 2>/dev/null || true
...
upload_category() {
    ...
    for file in "$local_path"/*.json; do
        if [ -f "$file" ]; then
            ...
            local temp_path="/home/$filename"
            
            # Copy file to container
            docker cp "$file" "$NAMENODE_CONTAINER:$temp_path"
            
            # Put file into HDFS
            docker exec $NAMENODE_CONTAINER hdfs dfs -put "$temp_path" "$hdfs_path/"
            
            # Clean up temp file (ignore errors)
            docker exec $NAMENODE_CONTAINER rm "$temp_path" 2>/dev/null || true
