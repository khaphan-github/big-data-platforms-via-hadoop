...
Do hạ tầng sử dụng các image Docker được xây dựng tùy chỉnh, nghiên cứu viên có thể tải trước các tệp dung lượng lớn trong `cluster/download/` bằng:

```bash
cd cluster/download && ./download.sh
```

Lệnh này là tùy chọn nếu máy có thể tải trực tiếp trong quá trình build image.

Khi khởi động hạ tầng bằng Docker Compose, các image nền tảng sẽ được xây dựng cùng các dịch vụ còn lại:

```bash
docker compose up -d --build
```

Quá trình build sẽ tạo ra hai image nền tảng:
